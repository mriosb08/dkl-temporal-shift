...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, tag_size, hidden_size,feat_size, emb_size,
        ...
        if depth > 1:
             self.lstm = nn.LSTM(emb_size, hidden_size, batch_first=True, num_layers=depth,
    ...
    def forward(self, x, seq_mask=None, seq_len=None):
        ...
        if self.aggregation_type == 'last_state':
            ...
            if self.bidirectional:
                ...
                h = final[-1]
            h = self.softplus (self.combination_layer(h))
            h = self.dropout_layer(h)
        elif self.aggregation_type == 'mean':
            outputs = self.dropout_layer(outputs)
            h = self.softplus (self.combination_layer(outputs))
            #h = outputs
            h = h.mean(dim=1)
        elif self.aggregation_type == 'sum':
            outputs = self.dropout_layer(outputs)
