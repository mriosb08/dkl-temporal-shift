...
max_time = 48.0 # set max num of hours
...
def tokenize(sent, mimic3_embedding=None, nlp=None):
    ...
    #clean sentence for deidentified tokens
...
def read_notes_table(mimic3_path):
    df_note = pd.read_csv(os.path.join(mimic3_path, 'NOTEEVENTS.csv')) #dataframe_from_csv(os.path.join(mimic3_path, 'NOTEEVENTS.csv')) #pd.read_csv(HP.mimic_note_events)
...
def filter_notes_on_stays(notes, stays):
    return pd.merge(notes, stays.drop_duplicates(), left_on=['SUBJECT_ID', 'HADM_ID'], right_on=['SUBJECT_ID', 'HADM_ID'], how='inner') #notes.merge(stays, how='inner', on=['SUBJECT_ID', 'HADM_ID'])
...
def break_up_notes_by_subject(notes, output_path, subjects=None, verbose=1):
    ...
    notes = notes[(notes['CHARTTIME'] < notes['DISCHTIME']) |
                                ((notes['CHARTDATE'] < notes['DISCHDATE']) &
                                 notes['CHARTTIME'].isnull())]
    notes = notes.drop(['DOB'],
                                     axis=1)
    ...
    for i, subject_id in enumerate(subjects):
        ...
        try:
            ...
            pass
        patient_note = notes.ix[notes.SUBJECT_ID == subject_id].sort_values(by=['CHARTDATE', 'CHARTTIME'])#.to_csv(os.path.join(dn, 'diagnoses.csv'), index=False)
        intime = patient_note.INTIME
        #print(intime)
        
        patient_note['HOURST'] = (patient_note.CHARTTIME - intime).apply(lambda s: s / np.timedelta64(1, 's')) / 60./60
        #print(subject_id)
        #print(patient_note['HOURS'])
