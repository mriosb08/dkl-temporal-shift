...
from mimic3benchmark.preprocessing import read_itemid_to_variable_map, map_itemids_to_variables, read_variable_ranges, remove_outliers_for_variable, clean_events
...
for subject_dir in os.listdir(args.subjects_root_path):
    ...
    #TODO??!! what is it doing remove outliers
    #var_ranges = read_variable_ranges(args.reference_range_file)
    #for v in variables:
    #    events = remove_outliers_for_variable(events, v, var_ranges)
    ...
    for i in range(stays.shape[0]):
        ...
        episodic_data.ix[episodic_data.index==stay_id].to_csv(os.path.join(args.subjects_root_path, subject_dir, 'episode{}_{}.csv'.format(i+1, stay_id)), index_label='Icustay')
        columns = list(episode.columns)
        columns_sorted = sorted(columns, key=(lambda x: "" if x == "Hours" else x))
        episode = episode[columns_sorted]
        episode.to_csv(os.path.join(args.subjects_root_path, subject_dir, 'episode{}_timeseries_{}.csv'.format(i+1, stay_id)), index_label='Hours')
